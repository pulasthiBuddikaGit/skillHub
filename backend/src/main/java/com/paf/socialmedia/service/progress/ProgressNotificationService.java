...
public class ProgressNotificationService {
    ...
    public ResponseEntity<?> getWorkoutNotifications(){
        List<ProgressNotification> workoutNotification = workoutNotificationRepository.findAll();
        return new ResponseEntity<List<ProgressNotification>>(workoutNotification, HttpStatus.OK);
    }
    public ResponseEntity<?> getUnreadWorkoutNotificationsByUserId(String userId) {
        List<ProgressNotification> workoutNotification = workoutNotificationRepository.findByUserIdAndIsReadFalse(userId);
        return new ResponseEntity<List<ProgressNotification>>(workoutNotification, HttpStatus.OK);
    }
    public ResponseEntity<?> saveWorkoutNotification(ProgressNotification workoutNotification){
        try{
            ...
            return new ResponseEntity<ProgressNotification>(workoutNotification, HttpStatus.OK);
        }catch(Exception e){
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    public ResponseEntity<?> updateWorkoutNotificationById(String id,ProgressNotification workoutNotification){

        Optional<ProgressNotification> existingNotification =  workoutNotificationRepository.findById(id);
